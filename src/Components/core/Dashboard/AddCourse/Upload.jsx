    ...
    <div>
      {image ? (
        <div className="flex flex-col space-y-2">
          ...
          <button  type="button"  onClick={() => { setImage(null); setValue(name, null); }}  className="text-sm text-richblack-5" >
            Remove
          </button>
        </div>
      ) : (
        <div className="flex flex-col space-y-2">
          ...
          {errors.courseImage && (  <span className="ml-2 text-xs tracking-wide text-pink-200">    Course Image is required**  </span> )}
        </div>
      )}
    </div>
  );
